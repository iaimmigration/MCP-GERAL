...
const Sidebar: React.FC<SidebarProps> = ({ onNewAgent, onOpenVocalArchitect, onGoHome, onOpenAdmin }) => {
  const { 
    agents, activeAgentId, tokenBalance,
    setActiveAgent, setCheckoutOpen, currentUser, logout
  } = useForgeStore();
    ...
    <div className="w-80 h-full border-r border-blue-900 bg-[#0a1128] flex flex-col shrink-0 overflow-hidden">
      <div className="p-8 border-b border-white/5 bg-black/20">
        ...
        {currentUser?.role === 'admin' && (
          <button 
            onClick={onOpenAdmin}
            className="w-full py-2 bg-amber-500/10 border border-amber-500/30 text-amber-500 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-amber-500/20 transition-all mb-4"
          >
      ...
      <div className="p-6 border-b border-white/5 bg-blue-900/10">
         <div className="flex items-center justify-between mb-3 px-1">
            ...
            <span className="text-[8px] font-bold uppercase text-blue-400">{currentUser?.role === 'admin' ? 'Root Access' : 'Enterprise'}</span>
         </div>
